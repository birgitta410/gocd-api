...
var pipelineRun = (function() {
  ...
  PipelineRunCreator.createNew = function (runData) {
    ...
    pipelineRun.summary = (function() {
      ...
      summary.withManualTrigger = function(triggerForced, triggerMessage) {
        if(! triggerForced) {
          return summary;
        }
        summary.changeInfo = summary.changeInfo || {};
        summary.changeInfo.comment = triggerMessage;
        summary.changeInfo.forced = true;
        return summary;
      };
      ...
      summary.withFilesChanged = function(files) {
        summary.changeInfo = summary.changeInfo || {};
        if(files == undefined) {
          summary.changeInfo.numberOfFilesModified = 0;
          summary.changeInfo.numberOfFilesAdded = 0;
          summary.changeInfo.numberOfFilesDeleted = 0;
          return summary;
        }

        summary.changeInfo.numberOfFilesModified = _.filter(files, { action: 'modified'}).length;
        summary.changeInfo.numberOfFilesAdded = _.filter(files, { action: 'added'}).length;
        summary.changeInfo.numberOfFilesDeleted = _.filter(files, { action: 'deleted'}).length;
        return summary;
      };
    ...
    function updateSummary() {
      pipelineRun.summary
        ...
        .withManualTrigger(pipelineRun['build_cause'].trigger_forced, pipelineRun['build_cause'].trigger_message)
