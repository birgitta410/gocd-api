...
function gocdRequestorModule() {
  ...
  var getPipelineDetails = function(pipelineId, pipelineName) {
    var pipelineDetailsPath = globalOptions.getGocdPipelineBasePath(pipelineName) + '/' + pipelineId + '.xml';
  ...
  var getHistory = function(offset, pipeline) {
    ...
    var historyPagePath = globalOptions.getGocdHistoryPath(pipeline) + (offset ? '/' + offset : '');
