...
function gocdRequestorModule() {
  ...
  function requestAndXml2Json(path) {
    return requestor.getFromGocd(path).then(function (body) {
  ...
  var getHistory = function(offset, pipeline) {

    var historyPagePath = getGocdHistoryPath(pipeline) + (offset ? '/' + offset : '');
    logger.debug('Requesting history', historyPagePath);

    return requestor.getFromGocd(historyPagePath).then(function (body) {
  ...
  var getPipelineNames = function() {
    ...
    return requestAndXml2Json(pipelinesPath).then(function (data) {
      console.log('requested pipeline data', data);
