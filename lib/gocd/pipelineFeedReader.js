  ...
  var refreshHistoryForPipeline = function (options, originalDefer) {
    ...
    requestHistoryAndDetails(offset, pipelineName).then(function (result) {
      function fillCache(runEntry) {
        ...
        var pipelineInCache = CACHE[pipelineName] && CACHE[pipelineName][runEntry.label];

        var cacheIsNewerThanRunEntry = pipelineInCache && CACHE[pipelineName][runEntry.label]['natural_order'] > runEntry['natural_order'];
