...
function pipelineFeedReaderModule() {
  ...
  function updateSummaryWithDependencies() {
    _.each(_.keys(CACHE.pipelineRuns), function(pipelineName) {
      _.each(_.keys(CACHE.pipelineRuns[pipelineName]), function(pipelineLabel) {
        var pipelineRun = CACHE.pipelineRuns[pipelineName][pipelineLabel];
        if(pipelineRun.summary.upstream) {
          var pipelineRuns = CACHE.pipelineRuns[pipelineRun.summary.upstream.pipelineName];
  ...
  var initFullHistoryCache = function(pipelineNames) {
    ...
    var pipelineRefreshes = _.map(pipelineNames, function (pipelineName) {
      CACHE.pipelineRuns[pipelineName] = CACHE.pipelineRuns[pipelineName] || {};
  ...
  var refreshHistoryForPipeline = function (options, originalDefer) {
    ...
    requestHistoryAndDetails(offset, pipelineName).then(function (result) {
      function fillCache(runEntry) {
        ...
        var pipelineInCache = CACHE.pipelineRuns[pipelineName] && CACHE.pipelineRuns[pipelineName][runEntry.label];

        var cacheIsNewerThanRunEntry = pipelineInCache && CACHE.pipelineRuns[pipelineName][runEntry.label]['natural_order'] > runEntry['natural_order'];
        ...
        if(! pipelineInCache) {
          ...
          CACHE.pipelineRuns[pipelineName][runEntry.label] = pipelineRun;

        } else {
          [CACHE.pipelineRuns[pipelineName][runEntry.label].update(runEntry)];
        }
      }

      function continueToNextPageOrResolve() {

        var cacheSize = _.keys(CACHE.pipelineRuns[pipelineName]).length;
  ...
  var readHistory = function(options) {
    ...
    return refreshHistoryForPipeline({ pipelineName: options.pipeline, offset: 0, maxNumberOfPages: 1}).then(function() {
      ...
      return {
        pipelineRuns: excludeSpecificRuns(CACHE.pipelineRuns[options.pipeline]),
