...
function pipelineFeedReaderModule() {
  ...
  var requestStages = function (offset) {
    return globalOptions.getHistoryRequestor().getHistory(offset).then(function(json) {
      if(json !== undefined) {
        var detailsPromises = _.map(json.pipelines, function (pipelineData) {
          return globalOptions.getHistoryRequestor().getPipelineDetails(pipelineData.id || pipelineData.counter);
        });
  ...
  var refreshData = function (offset, originalDefer) {

    var defer = originalDefer || Q.defer();

    requestStages(offset).then(function (result) {
  ...
  var readPipelineRuns = function(options) {
    ...
    return _.omit(CACHE, function (value, key) {
