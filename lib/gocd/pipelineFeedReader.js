...
function pipelineFeedReaderModule() {
  ...
  function addStatistics() {
    ...
    _.each(_.keys(CACHE), function (pipelineName) {
      ...
      var passedRunsWithTime = _.chain(CACHE[pipelineName].pipelineRuns)
        .map(function(pipelineRun) {

          if(pipelineRun.label === undefined) {
            return undefined;
          }
        ...
        .filter(function(pipelineResult) {
          return pipelineResult.wasSuccessful();
        })
      ...
      if (lastSuccessfulRun === undefined) {
        ...
        CACHE[pipelineName].statistics.timeSinceLastSuccess = {
          human: moment(lastSuccessfulRun.lastScheduled).fromNow(),
          milliSeconds: lastSuccessfulRun.lastScheduled
        };
      }
