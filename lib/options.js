...
function optionsModule() {
  ...
  var getHistoryRequestor = function() {
    if(useSnap()) {
      ...
      console.log('========= NO SNAP', options.type);
  ...
  var getCcTrayUrl = function(pipeline) {
    if(useSnap()) {
      return 'https://snap-ci.com/' + options.user + '/project/' + (pipeline || options.pipeline) + '/branch/master/cctray.xml';
    } else {
      return baseUrl() + '/go/cctray.xml';
    }
  };
